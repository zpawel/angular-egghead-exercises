...
module.exports = function ()
{
    ...
    this.Then(/^The element "([^"]*)" should have class "([^"]*)"$/, function (name, nameClass, callback)
    {
        expect(fragments(name)().getAttribute('class')).to.eventually.have.string(nameClass).and.notify(callback);
    });
    this.Then(/^The element "([^"]*)" should NOT have class "([^"]*)"$/, function (name, nameClass, callback)
    {
        expect(fragments(name)().getAttribute('class')).to.not.eventually.have.string(nameClass).and.notify(callback);
    });
