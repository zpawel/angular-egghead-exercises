...
module.exports = function ()
{
    ...
    this.Then(/^I should see "([^"]*)" in "([^"]*)"$/, function (cssClass, element, callback)
    {
        expect(fragments(element)().getAttribute('class')).to.become(cssClass).and.notify(callback);
    });

    this.Then(/^I should see "([^"]*)" message in "([^"]*)"$/, function (text, result, callback)
    {
    ...
    this.Then(/^I should not see message in "([^"]*)"$/, function (message, callback) {
        expect(fragments(message)().getText()).to.become('').and.notify(callback);
    });
